...
for event in all_events:
    ...
    current = dedup[key]
    memberships = current.setdefault("sourceTeams", [current.get("team", "")])
    for team in event.get("sourceTeams", [event.get("team", "")]):
        if team and team not in memberships:
            memberships.append(team)

    # Fill any useful blank values from the duplicate record.
    for field in ("away", "home", "venue", "title", "league", "booking",
                  "reserveButtonStatus", "productId", "link"):
